...
class HDHRConfig():
    ...
    config = {
                ...
                "tvheadend": {
                              ...
                              "streamProfile": "pass",  # specifiy a stream profile that you want to use for adhoc transcoding in tvh, e.g. mp4
    ...
    def config_adjustments(self, script_dir):
        ...
        if not self.config["tvheadend"]["username"] or not self.config["tvheadend"]["username"]:
            print("Tvheadend Login Credentials Missing. Exiting...")
            clean_exit()
        ...
        tvheadend_cache = pathlib.Path(cache_dir).joinpath('tvheadend')
        self.config["main"]["tvheadend_cache"] = tvheadend_cache
        if not tvheadend_cache.is_dir():
            tvheadend_cache.mkdir()
        self.config["tvheadend"]["epg_cache"] = pathlib.Path(tvheadend_cache).joinpath('epg.json')
