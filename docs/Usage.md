...
---
[Main](README.md)  |  [Setup and Usage](Usage.md)  |  [TVheadend](Origin.md)  |  [Credits/Related Projects](Related-Projects.md)
---
**f**un
**H**ome
**D**istribution
...
* this guide assumes we wish to use the `~/fhdhr` directory for our install (you can use whatever directory you like, just make the appropriate changes elsewhere in this guide) and that we are installing for TVheadend support
* run the following commands to clone the repo into `~/fhdhr/fHDHR_TVheadend`
```
cd ~/fhdhr
git clone https://github.com/fHDHR/fHDHR_TVheadend.git
```
* create your config.ini file (as described earlier in this guide) in the `~/fhdhr/fHDHR_TVheadend` directory
...
services:
  tvheadend:
    build: ./fHDHR_TVheadend
    container_name: tvheadend
    network_mode: host
    volumes:
      - ./fHDHR_TVheadend/config.ini:/app/config/config.ini
```
* run the following command to build and launch the container
```
docker-compose up --build -d tvheadend
```
